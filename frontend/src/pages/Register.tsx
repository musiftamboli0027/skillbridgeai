...
export default function Register() {
  ...
  // Support for ?role=admin (handling HashRouter quirks)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await register(name, email, password, targetRole || 'student');
      if (success) {
        // Backend now returns success message for verification flow
        // We can show a state or just use a simple alert/message for now
        setError('');
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          <h1 className="text-2xl font-bold text-black mb-2 text-center">
            {targetRole === 'admin' ? 'Admin Registration' : 'Create Account'}
          </h1>
          <p className="text-[#333333] text-center mb-6">
            {targetRole === 'admin'
              ? 'Join the SkillBridge elite squad'
              : 'Start your tech journey with SKILLBRIDGE'}
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-start gap-3">
              <button
                ...
                className={`mt-0.5 w-5 h-5 rounded border flex items-center justify-center transition-colors ${agreeTerms ? 'bg-[#8b5cf6] border-[#8b5cf6]' : 'border-black/20'
                  }`}
              >
              ...
              <p className="text-sm text-[#333333]">
                I agree to the{' '}
                <Link to="#" className="text-[#8b5cf6] hover:underline">Terms of Service</Link>
                {' '}and{' '}
                <Link to="#" className="text-[#8b5cf6] hover:underline">Privacy Policy</Link>
              </p>
            </div>
